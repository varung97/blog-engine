allPostsApp = angular.module("allPostsApp", ['ngRoute', 'shared-services']);

allPostsApp.config(function($routeProvider) {
		$routeProvider
			.when('/', {
				templateUrl : 'all_posts_posts.html',
...
allPostsApp.factory('Data', function(){
  return {
    Post_ID : 1,
    username: null,
    page_number: 1,
...
allPostsApp.controller("navbarCtrl", function($scope, $http, SharedServices) {
...
allPostsApp.controller("allPostsCtrl", function($scope, $http, SharedServices, $anchorScroll, $location, Data){
  $scope.num_pages = 5;
  ...
  $scope.$on('$includeContentLoaded', function () {
    angular.element('#nav_all_posts').addClass('active').children().attr('href', '#').attr('onclick', 'window.location.reload()');
...
allPostsApp.controller("viewPostCtrl", function($scope, $http, SharedServices, Data){
